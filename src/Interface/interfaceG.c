	...
	for(i=1; i<=max; i++){
			...
			for(j=0; j < data->liste_objets->nb_objets; j++){
					...
					if(data->liste->objets[j] == i){
							...
							int color_degre = 200/max;
							int color_degre2 = 150/max;
							//TEST ORIENTATION OBJ
							ez_set_color (ez_get_RGB(i*color_degre%255+50, 0, i*color_degre2%255+100));
							if(data->liste->orientation[j] == 0){
									...
									ez_draw_text(ev->win, EZ_TC, (x_start+(x_start+l_obj*data->scale))/2, (y_start+(y_start-h_obj*data->scale))/2, "%d",data->liste->orientation[j]);
									//MAJ DIMENSIONS
									...
									ez_set_color (ez_get_RGB(255, 255, 255));
									ez_draw_text(ev->win, EZ_TC, (x_start+(x_start+h_obj*data->scale))/2, (y_start+(y_start-l_obj*data->scale))/2, "%d",data->liste->orientation[j]);
...
void gui(boite *box, Liste *A, Liste *D, Liste_objet *L, double temps, double temps2){
	...
	data2.box = box;
	data2.liste = D;
	data2.liste_objets = L;
	data2.scale=10;
	data2.temps=temps2;

	win1 = ez_window_create (box->largeur*10+100, box->hauteur*10+100, "Packing 2000 - A", win1_on_event);
	ez_window_dbuf(win1,1);
	ez_set_data(win1, &data);
	
	win2 = ez_window_create (box->largeur*10+100, box->hauteur*10+100, "Packing 2000 - D", win1_on_event);
	ez_window_dbuf(win2,1);
	ez_set_data(win2, &data2);
